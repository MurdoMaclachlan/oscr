...
def check_args() -> None:
    ...
    # If any of the closing args, i.e. args like "help" or "version" that show a
    # specific output have been run, this flag tells it not to process non-closing args
    # and to kill the program after it's done.
    closing = False
    ...
    for argument in arguments:
        if argument in sys.argv[1:]:
            # If argument changes config and is not closing, will call its
            # appropriate function
            if not closing and argument in config_changes:
                # noinspection PyArgumentList
                arguments[argument](config_changes[argument])
                if argument in ["--clean-hunt", "-C"] and sys.argv.index(
                    argument
                ) != len(sys.argv):
                    print(
                        Log.new(
                    ...
                    break
            # If a closing argument and an argument that will be overwritten by it were
            # both passed, warns user
            elif (
...
"""
    Below are listed the function definitions for each argument.

    They functions are listed in alphabetical order of their names; not in alphabetical
    order of the argument name, nor in order of run-time priority.

    For run-time priority, see their order in the 'arguments' dictionary in checkArgs(),
    above.
"""
...
def clean_hunt() -> None:
    ...
    temp_config_change(
        [
            ...
            (
                ["", ""]
                if Globals.get(key="user_list") == ["transcribersofreddit"]
                else ["user_list", ["transcribersofreddit"]]
            ),
        ]
    )
...
def help_menu() -> None:
    ...
    print(
        "List of Arguments:\n",
        "--clean-hunt, -C:    runs an isolated instance of OSCR that deletes ToR bot",
        "interactions containing the phrase 'treasure hunt'\n",
        "--credits, -c:       lists everyone who has helped with the creation of the",
        "program\n",
        "--force-regex, -f:   forces the program to enable regex for one instance",
        "regardless of configuration\n",
        "--help, -h:          displays this list\n",
        "--no-recur, -n:      forces program to run only one cycle regardless of",
        "'recur' configuration\n",
        "--print-logs, -p:    forces program to print logs for one instance regardless",
        "of 'printLogs' configuration\n",
        "--reset-config, -R:  resets the config file to defaults\n",
        "--report-totals, -r: forces program to report total statistics for one",
        "instance regardless of 'reportTotals' configuration\n",
